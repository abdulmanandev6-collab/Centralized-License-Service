...
from licenses.models import Activation, License
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "instance_id": openapi.Schema(
                    ...
                    description="Your site URL, domain, or machine ID where the product is installed (e.g., https://mysite.com, mysite.com, or machine-12345)",
                    example="https://mysite.com",
                ),
                "product_slug": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="Product slug from your license (e.g., rankmath, wprocket). Check your license status to see available products.",
                    example="rankmath",
                ),
            },
            example={"instance_id": "https://mysite.com", "product_slug": "rankmath"},
        ),
        responses={
            201: "Success",
            200: "Success",
    ...
    def post(self, request):
        ...
        try:
            license_obj = License.objects.filter(
                license_key=lk, product__slug=product_slug, product__is_active=True
            ).first()
            ...
            with transaction.atomic():
                existing = Activation.objects.filter(
                    license=license_obj, instance_id=instance_id, is_active=True
                ).first()
...
class CheckLicenseStatusView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            licenses_data = []
            for lic in licenses:
                active_count = Activation.objects.filter(license=lic, is_active=True).count()
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "instance_id": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    description="The instance to deactivate",
                    example="https://oldsite.com",
                ),
                "product_slug": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Product slug", example="rankmath"
                ),
            },
            example={"instance_id": "https://oldsite.com", "product_slug": "rankmath"},
        ),
        responses={
            200: "Success",
            404: "Not found",
        ...
        try:
            ...
            with transaction.atomic():
                act = Activation.objects.filter(
                    license=lic, instance_id=instance_id, is_active=True
                ).first()
            ...
            return Response(
                {"error": "Failed to deactivate seat"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
