...
class BrandAPIPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return hasattr(request, 'user') and request.user is not None and isinstance(request.user, type(request.user)) and hasattr(request.user, 'api_key')
...
class LicenseKeyPermission(permissions.BasePermission):
    ...
    """
    def has_permission(self, request, view):
        return hasattr(request, 'user') and request.user is not None and hasattr(request.user, 'key')
